...
Last updated: 2026-06-17. Scope: everything in `reapp-protocol` except generated
and vendored paths (`node_modules/`, `dist/`, `target/`, `*.wasm`) and local
secrets. Status legend:
...
| `test_snapshots/` (20 JSON) | Auto-generated Soroban test snapshots. Gitignored; regenerated by `cargo test`. | Cut candidate (autogen) |
...
|---|---|---|---|
| `deploy.mjs` | `deploy:testnet` | Builds the contract to WASM and deploys MandateRegistry; writes the contract id back to `.env`. | Keep |
...
| `.gitignore`, `.env.example` | Ignore rules and env template. | Keep |
| `.env` | Local secrets (gitignored). Burner keys. | Keep, never commit |
| `.claude/settings.local.json` | Local permission allowlist (gitignored). | Keep |
...
5. **Autogen, gitignored.** `contracts/.../test_snapshots/` regenerates on
   `cargo test`.
