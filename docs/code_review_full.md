...
- [Scripts & Tooling](#scripts-tooling)
  ...
  - [`scripts/lib/env.ts`](#scriptslibenvts)
...
| 44 | [`scripts/lib/env.ts`](#scriptslibenvts) | 83 | Shared script bootstrap: loads .env from the repo root as an import side effect and provides uniform, secret-masking verbose logging used across the script suite. |
...
*Shared script bootstrap: loads .env from the repo root as an import side effect and provides uniform, secret-masking verbose logging used across the script suite.*

This module is the common foundation for the TypeScript-aware scripts. Importing it loads `.env` from the repo root (`../../.env` relative to `scripts/lib/`, cwd-proof) as a side effect, and if `VERBOSE` is on (the default; set `VERBOSE=0` to quiet) it immediately prints dotenv load status and the parsed key names. It exports the `VERBOSE` flag plus a small logging vocabulary - `log` (always), `debug` (only when VERBOSE, prefixed with ` · `), and `step` (a `▶`-prefixed header) - that gives every script the same scannable output style.

The security-relevant piece is `mask(key, value)`, driven by two regexes. `PHRASE` (`/MNEMONIC|SEED/i`) collapses seed/mnemonic values to a bullet string plus word count, never showing the value. `SENSITIVE` (`/SECRET|PRIVATE|PASSWORD|TOKEN|(^|_)KEY($|_)/i`) renders secrets as first-4…last-4 (or `••••` if ≤8 chars). Everything else (URLs, public keys, network passphrase) is shown in full. A deliberate comment notes the regex must NOT match `NETWORK_PASSPHRASE` (a public value), which is why there's no bare `PHRASE` token and `KEY` is anchored with separators.

`ENV_KEYS` enumerates the full set of env vars REAPP scripts read, and `printEnvBanner(scriptName, keys)` prints a masked banner of those values (or `(unset)`), padded for alignment, plus the VERBOSE state. This gives operators a one-glance, safe view of the environment a script is running under without leaking secrets.

**Key items:**

- `VERBOSE` — Boolean export; true unless `VERBOSE=0` - gates `debug()` and the import-time dotenv banner.
- `log(...args)` — Always-printed console.log wrapper.
- `debug(...args)` — Prints only when VERBOSE, prefixed with ` · ` so it's scannable.
- `step(title)` — Prints a `▶`-prefixed step header.
- `SENSITIVE / PHRASE` — Regexes classifying env keys: PHRASE → word-count only; SENSITIVE → first-4…last-4 masking; deliberately excludes NETWORK_PASSPHRASE.
- `mask(key, value)` — Returns a masked representation of a value based on its key's sensitivity class; full value for non-sensitive keys.
- `ENV_KEYS` — The canonical list of env vars REAPP scripts read (network, RPC, passphrase, burner keys, contract id, USDC SAC).
- `printEnvBanner(scriptName, keys)` — Prints a masked, aligned banner of the listed env vars (or `(unset)`) plus the VERBOSE state.

> **Note:** Masking correctness hinges on the two regexes. `SENSITIVE` anchors `KEY` as `(^|_)KEY($|_)` and avoids a bare `PHRASE` token specifically so `NETWORK_PASSPHRASE` (a public value) is shown in full rather than masked - a subtle but intentional rule called out in the source comment. Note that, unlike `RESULT` in the other dotenv loaders, this `dotenv.config` call omits the `quiet` option, so dotenv may emit its own load tip; and merely importing the module has the side effect of loading .env and (when VERBOSE) printing a banner, which callers should expect.
...
The demo playbook and the repo-level configuration: workspace package.json, the shared tsconfig base, the GitHub Actions CI workflow, the pre-push hook, the env template, and the top-level README.

### `playbook/demo.ts`

*The `npm run demo` entry point - a thin wrapper that shells out to the canonical on-chain end-to-end testnet script.*

This is the executable behind `npm run demo` (mapped in package.json to `tsx playbook/demo.ts`). It contains no demo logic of its own; it is a delegating launcher. The module header documents the REAPP "aha" demo (referenced as §9): the happy path (user authorizes a mandate, agent pays, 1 XLM moves) followed by three rogue-agent rejections that the *contract* enforces rather than the SDK - overspend yields `BudgetExceeded`, replay yields `BadSequence`, and pay-after-revoke yields `MandateRevoked`. The stated lesson is that the SDK is untrusted and the spending limit lives in the on-chain `MandateRegistry` contract.

Mechanically, it resolves its own directory from `import.meta.url` (via `fileURLToPath` + `path.dirname`), then computes the absolute path to `../scripts/e2e-testnet.mjs`. It launches that script synchronously with `spawnSync("node", [e2e, ...argv])`, forwarding any CLI arguments the user passed (`process.argv.slice(2)`) and inheriting stdio so the child's verbose narration streams straight to the terminal. It exits with the child's status code, defaulting to `1` if the child produced no status (a null exit, e.g. killed by signal).

It connects to the wider system purely by reference: all real behavior - RPC calls, transaction building, the rogue-case assertions - lives in `scripts/e2e-testnet.mjs`, which is the same script also exposed directly as `npm run e2e:testnet`. So `demo` and `e2e:testnet` run identical code; the wrapper exists to give the demo a friendly named npm script and a documented narrative header.

**Key items:**

- `here (const)` — Absolute directory of this module, derived from `path.dirname(fileURLToPath(import.meta.url))` so resolution is independent of cwd.
- `e2e (const)` — Absolute path to the delegated script `../scripts/e2e-testnet.mjs`, resolved relative to `here`.
- `res (const) / spawnSync call` — Runs `node e2e-testnet.mjs <forwarded args>` synchronously with inherited stdio so child output streams live.
- `process.exit(res.status ?? 1)` — Propagates the child's exit code; falls back to non-zero (1) when status is null so a killed/failed child never reports success.

> **Note:** Exit-code fidelity matters here: because CI and the pre-push hook treat a nonzero exit as failure, the `?? 1` fallback ensures a signal-killed child is not silently reported as a passing demo. The wrapper trusts that `scripts/e2e-testnet.mjs` exists at the relative path; there is no existence check, so a moved/renamed script would surface only as Node's module-not-found error from the child.
...
*Root workspace manifest - defines the npm monorepo, the canonical run/build/verify/demo scripts, and shared dev tooling.*

This is the private root package for the `reapp-protocol` monorepo (`"private": true`, version `0.0.0`). Its `workspaces` globs (`packages/*`, `apps/*`) make it an npm workspaces root, so a single `npm ci`/`npm install` hoists dependencies for every package and app (the SDK packages, the fulfillment/consumer reference agents, etc.). The description states the thesis: AP2 mandates enforced on-chain by Soroban via x402.

The `scripts` block is the operator's command surface. `build` deliberately builds `@reapp-sdk/stellar` first (the typed Soroban layer that other packages depend on) and then fans out to all workspaces with `--if-present`. `test` and `typecheck` similarly cover the whole tree (`tsc --build` walks project references). `verify` runs `scripts/verify.mjs` - the local CI-equivalent gate invoked by the pre-push hook. The remaining scripts map to the operational toolset in `scripts/` and `playbook/`: `demo` (tsx wrapper over the on-chain e2e), `keys:derive-freighter`, `deploy:testnet`, `e2e:testnet`, `e2e:sdk`, `e2e:x402` (the x402 round-trip, run via tsx), and `audit` (the independent on-chain mandate auditor).

Dependency-wise it is tooling-heavy: `devDependencies` pin the Stellar SDK (`@stellar/stellar-sdk ^14.6.1`), `@types/node`, `dotenv`, `playwright`, `stellar-hd-wallet` (used by the Freighter key-derivation script), `tsx`, and `typescript ^5.7.2`. The single runtime `dependency` is `@reapp-sdk/core ^0.1.2` - pulled from the npm registry (the published package, per the README) rather than the local workspace copy, which lets the root scripts exercise the released SDK.

**Key items:**

- `workspaces` — `packages/*` and `apps/*` - declares the npm workspaces root so all SDK packages and reference apps share one install/hoist.
- `scripts.build` — Builds `@reapp-sdk/stellar` first (dependency ordering), then `--workspaces --if-present` builds the rest.
- `scripts.test` — Runs each workspace's test script if present; invoked by CI's `npm test`.
- `scripts.typecheck` — `tsc --build` - type-checks the whole project-reference graph without emitting via the run scripts.
- `scripts.verify` — `node scripts/verify.mjs` - the local CI mirror (fmt/clippy/test/build) that the pre-push hook executes.
- `scripts.demo` — `tsx playbook/demo.ts` - runs the on-chain aha demo (happy path + rogue rejections).
- `scripts.keys:derive-freighter` — `node scripts/derive-freighter.mjs` - scans BIP39 indexes to match a Freighter public key from a seed typed at runtime.
- `scripts.deploy:testnet` — `node scripts/deploy.mjs` - deploys the MandateRegistry contract to Stellar testnet.
- `scripts.e2e:testnet` — `node scripts/e2e-testnet.mjs` - the canonical on-chain e2e; same script the demo wrapper delegates to.
- `scripts.e2e:sdk` — `node scripts/e2e-sdk.mjs` - exercises the SDK flow end to end on testnet.
- `scripts.e2e:x402` — `tsx scripts/e2e-x402.ts` - the full x402 round-trip (ResearchAgent buys from the 402-gated merchant, budget enforced).
- `scripts.audit` — `node scripts/audit-mandate.mjs` - independent on-chain mandate auditor.
- `devDependencies` — Tooling pins: `@stellar/stellar-sdk ^14.6.1`, `@types/node ^22`, `dotenv`, `playwright`, `stellar-hd-wallet`, `tsx`, `typescript ^5.7.2`.
- `dependencies` — Single runtime dep `@reapp-sdk/core ^0.1.2` - resolves to the published npm package, exercising the released SDK from root scripts.

> **Note:** The root depends on `@reapp-sdk/core` from the registry (`^0.1.2`), not on the in-repo workspace via `*`/`workspace:` - so root scripts run against the *published* SDK, which can drift from the local `packages/sdk` source if the local copy is ahead of the last publish. `build` hardcodes `@reapp-sdk/stellar` as the first build step to satisfy dependency ordering; adding a new package that other packages depend on before it would need the same manual prepend, since `--workspaces` ordering is otherwise unspecified.
...
```json
{
  ...
  "version": "0.0.0",
  "private": true,
  "description": "Agent-driven payments: AP2 mandates enforced on-chain by Soroban via x402.",
  "workspaces": [
    "packages/*",
    "apps/*"
  ],
  "scripts": {
    "build": "npm run build -w @reapp-sdk/stellar && npm run build --workspaces --if-present",
    "test": "npm run test --workspaces --if-present",
    "typecheck": "tsc --build",
    "verify": "node scripts/verify.mjs",
    "demo": "tsx playbook/demo.ts",
    "keys:derive-freighter": "node scripts/derive-freighter.mjs",
    "deploy:testnet": "node scripts/deploy.mjs",
    "e2e:testnet": "node scripts/e2e-testnet.mjs",
    "e2e:sdk": "node scripts/e2e-sdk.mjs",
    "e2e:x402": "tsx scripts/e2e-x402.ts",
    "audit": "node scripts/audit-mandate.mjs"
  },
  "devDependencies": {
...
*Shared TypeScript compiler baseline that per-package tsconfigs extend.*

This is the single source of truth for compiler settings across the monorepo; individual packages `extends` it and add their own `include`/`outDir`/`references`. It targets modern Node: `target: ES2022` with the `ES2022` lib, and `module`/`moduleResolution` both set to `NodeNext`, which enforces ESM-style resolution semantics (explicit file extensions, package `exports` honored) consistent with the `.mjs`/`.ts` ESM scripts used throughout the repo.

It turns on the full strictness and library-publishing surface needed for a typed SDK that ships to npm: `declaration` + `declarationMap` emit `.d.ts` and their source maps so downstream consumers get types and go-to-definition; `sourceMap` emits JS source maps; `strict` enables the whole strict family; and `noUncheckedIndexedAccess` adds `undefined` to indexed-access results, forcing callers to handle missing array/record entries (important for on-chain data parsing where fields may be absent). `esModuleInterop` smooths CommonJS default-import interop, `forceConsistentCasingInFileNames` prevents case-mismatch import bugs on case-insensitive filesystems (macOS dev, Linux CI), and `skipLibCheck` skips type-checking of dependency `.d.ts` files to keep builds fast.

It has no `include`, `files`, `outDir`, or `composite`/`references` of its own - it is purely a base. The root `npm run typecheck` (`tsc --build`) relies on each extending package to declare project references and emit settings on top of this baseline.

**Key items:**

- `target / lib: ES2022` — Compiles to and types against ES2022, matching the Node 22 runtime used in CI and scripts.
- `module / moduleResolution: NodeNext` — ESM-first resolution honoring package `exports` and explicit extensions, consistent with the repo's `.mjs`/ESM `.ts`.
- `declaration + declarationMap` — Emits `.d.ts` and declaration source maps so the published SDK packages ship usable types and navigation.
- `sourceMap` — Emits JS source maps for debuggable compiled output.
- `strict` — Enables the full strict type-checking family (null checks, implicit-any, strict function types, etc.).
- `noUncheckedIndexedAccess` — Adds `undefined` to indexed lookups, forcing explicit handling of possibly-absent elements - safer on-chain data parsing.
- `esModuleInterop` — Enables synthetic default imports for CommonJS modules.
- `skipLibCheck` — Skips type-checking of dependency declaration files for faster builds.
- `forceConsistentCasingInFileNames` — Rejects case-inconsistent imports, preventing macOS-vs-Linux casing breakage.

> **Note:** `skipLibCheck: true` is a deliberate speed/safety tradeoff: type errors inside third-party `.d.ts` (including `@stellar/stellar-sdk`) are not surfaced, so a dependency's broken types could slip through CI. `noUncheckedIndexedAccess` is on, so any code in extending packages that indexes arrays/records must narrow away `undefined` or the strict build (`npm run typecheck` / CI `npm run build`) fails.
...
*GitHub Actions CI pipeline - runs the Rust contract suite (incl. the §10 negative suite) and the TypeScript workspaces on every push and PR.*

This workflow (named `ci`) triggers on every `push` and every `pull_request` with no branch filter, so it runs on all branches from the first commit. The header comment encodes a project invariant from the Stellar Foundation: the §10 negative suite must run from commit one and must never be "added at the end" - i.e. the adversarial/rejection tests are a permanent gate, not an afterthought.

It defines two independent parallel jobs. The `contract` job (`mandate-registry (Rust)`) runs on `ubuntu-latest` with its working directory pinned to `contracts/mandate-registry`. It checks out the repo, installs the stable Rust toolchain via `dtolnay/rust-toolchain@stable` adding the `wasm32-unknown-unknown` target (the Soroban Wasm build target), enforces formatting with `cargo fmt --all -- --check`, then runs `cargo test`, which includes the negative suite (overspend/replay/revoke rejection tests). The `packages` job (`workspaces (TypeScript)`) checks out, sets up Node 22, runs `npm ci` for a clean reproducible install, `npm run build` (which builds `@reapp-sdk/stellar` first then all workspaces), and `npm test` across the workspaces.

This CI is mirrored locally: `scripts/verify.mjs` (invoked by `npm run verify` and the `.githooks/pre-push` hook) is the local equivalent so no push should reach CI red. Note the WASM target is added but the workflow does not separately run a `cargo build --target wasm32...`; the target is present so toolchain/test steps that touch Wasm resolve, while `cargo test` runs the native test binary.

**Key items:**

- `on: push / pull_request` — Unfiltered triggers - CI runs on every push and PR across all branches.
- `job: contract (mandate-registry Rust)` — Ubuntu job rooted at `contracts/mandate-registry` that fmt-checks and tests the enforcement contract.
- `dtolnay/rust-toolchain@stable + wasm32-unknown-unknown` — Installs stable Rust with the Soroban Wasm compile target.
- `cargo fmt --all -- --check` — Fails CI on any unformatted Rust code.
- `cargo test (incl. negative suite)` — Runs the contract unit tests including the §10 rogue-case rejection tests (the mandated gate).
- `job: packages (workspaces TypeScript)` — Ubuntu/Node-22 job that installs, builds, and tests all TS packages and apps.
- `npm ci` — Clean, lockfile-pinned install for reproducible CI builds.
- `npm run build / npm test` — Builds the dependency-ordered workspace graph, then runs all workspace tests.

> **Note:** Invariant encoded in the header: the negative/adversarial suite is part of `cargo test` from commit one and must stay there - removing or deferring it violates a Stellar directive. The two jobs run independently with no `needs:` dependency, so a Rust-only failure and a TS-only failure are reported separately. There is no explicit Cargo/npm dependency caching step, so runs are reproducible but not optimized for speed.

```yaml
name: ci

# The §10 negative suite runs on every push from commit one — a Stellar
# directive. It must never be "added at the end".
...
jobs:
  contract:
    name: mandate-registry (Rust)
    runs-on: ubuntu-latest
    defaults:
      run:
        working-directory: contracts/mandate-registry
    steps:
      ...
      - name: cargo test (incl. negative suite)
        run: cargo test

  packages:
    name: workspaces (TypeScript)
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
...
*Git pre-push hook that runs the full local CI-equivalent gate before any push is allowed.*

A minimal POSIX `sh` hook whose sole job is to block pushes that would fail CI. It `exec`s `npm run verify`, replacing the shell process so the hook's exit status is exactly that of the verify run - a nonzero exit aborts the push. `npm run verify` maps to `node scripts/verify.mjs`, which is the local mirror of the GitHub Actions pipeline (Rust fmt + tests including the negative suite, and the TypeScript build + tests).

It is not active by default: Git only consults `.git/hooks`, so the hook is enabled by repointing Git at this tracked directory with `git config core.hooksPath .githooks` (documented inline). Tracking the hook in-repo under `.githooks/` makes it shareable and reviewable rather than a per-developer local file.

This is the human-side enforcement of the "every push must be green" rule: by running the same checks locally that CI runs remotely, it prevents red CI runs from ever being triggered, keeping the remote pipeline always-green.

**Key items:**

- `#!/bin/sh shebang` — POSIX shell hook for maximum portability across developer environments.
- `exec npm run verify` — Replaces the shell with the verify run so the hook's exit code is the gate's exit code - nonzero aborts the push.
- `core.hooksPath enablement (comment)` — Documents the one-time `git config core.hooksPath .githooks` needed to activate the tracked hook.

> **Note:** The hook is opt-in: until a developer runs `git config core.hooksPath .githooks` it does nothing, so it is a convention-backed gate, not a hard guarantee - CI remains the actual enforcement boundary. Using `exec` means no cleanup or post-processing runs after `verify`; the hook is intentionally a pure pass-through of verify's status.

```bash
#!/bin/sh
# REAPP pre-push gate: run the full CI-equivalent locally so no push can fail CI.
